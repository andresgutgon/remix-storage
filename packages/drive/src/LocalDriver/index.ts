 ...
 * Investigate how to mimic this AdonisJS routing but in Remix
 */
class RouterContract {
  makeUrl(routeName: string, _options: any): string {
    return routeName
  }
  makeSignedUrl(routeName: string, _params: any, _options: any): string {
    return routeName
  }
}
  ...
  public async copy(source: string, destination: string): Promise<void> {
    try {
      await this.adapter.copy(
        this.makePath(source),
        this.makePath(destination),
        {
          overwrite: true,
        }
      )
